...
@MainActor final class FloatingControl {
    ...
    init(store: Store) {
        self.store = store
        panel = VoicePanel(
            contentRect: NSRect(x: 12, y: 350, width: 48, height: 184),
        ...
        let hosting = NSHostingView(
            rootView: FloatingView(appearance: appearance)
    ...
    func followPointer(force: Bool = false) {
        ...
        if force || screen != currentScreen {
            currentScreen = screen
            lastHover = .distantPast
        }
    ...
    func updateVisibility() {
        ...
        let active =
            ...
            store.busy ||
            store.error != nil ||
            store.widgetNotice != nil ||
            store.correctionSuggestion != nil

        guard active || store.preferences.showWidgetAlways != false else {
            panel.orderOut(nil)
            return
        }
        ...
        let collapsed =
            !active &&
            store.preferences.autoHideWidget != false &&
            !menuOpen &&
            Date().timeIntervalSince(lastHover) > 0.9

        let width: CGFloat = collapsed ? 10 : active ? 280 : 48
        let height: CGFloat = collapsed ? 48 : 184

        let frame = NSRect(
            x: screen.visibleFrame.minX + (collapsed ? 2 : 10),
...
struct FloatingView: View {
        ...
        Group {
            if appearance.collapsed {
                Capsule()
                    ...
                    .accessibilityHint("Move the pointer here to show dictation controls")
            } else {
                controls
                    .frame(
                        width:
    ...
    var controls: some View {
        HStack(spacing: 8) {
            VStack(spacing: 6) {
                Menu {
                    Button("Multilingual · English + Romanian") {
                        s.preferences.locale = "auto"
                        s.save()
                    }

                    Button("English") {
                        s.preferences.locale = "en-US"
                        s.save()
                    }

                    Button("Română") {
                        s.preferences.locale = "ro-RO"
                        s.save()
                    }
                } label: {
                ...
                Button {
                    s.toggle(kind: "Dictation", fromHotkey: true)
                } label: {
                    Image(systemName: s.recording ? "stop.fill" : "mic.fill")
                        .foregroundStyle(s.recording ? Color.red : .white)
                ...
                .accessibilityHint(
                    s.recording
                        ? "Stops the current dictation"
                        : "Starts recording a dictation"
                )
                .disabled(s.busy)

                Button {
                    s.toggle(kind: "Notetaker")
                } label: {
                ...
                s.correctionSuggestion != nil
            {
                VStack(alignment: .leading, spacing: 7) {
                    HStack(spacing: 7) {
                        Circle()
                            .fill(s.recording ? Color.red : .mint)
                            .frame(width: 5, height: 5)
                            .accessibilityHidden(true)

                        Text(
                            s.recording
                                ? (s.handsFree ? "Hands-free" : "Listening")
                                : s.busy
                                ? "Processing"
                                : s.error != nil
                                ? "Needs attention"
                    ...
                    if s.error != nil && !s.recording && !s.busy {
                        ...
                        let correction = s.correctionSuggestion,
                        !s.recording &&
                        !s.busy
                    {
    ...
    var recordingFooter: String {
        guard s.recording else {
            return s.error != nil
                ? "Your audio is saved"
                : s.correctionSuggestion != nil
                ? "Available for 8 seconds"
                : "This message will close automatically"
        }

        let control =
            s.handsFree
                ? "Press \(s.shortcutName) to finish"
                : "Release to finish · double-tap to lock"

        guard s.streamedPhraseCount > 0 else {
            return control
        }
...
struct ShortcutSheet: View {
    ...
    @State var hint =
        "Press a key combination, or click an extra mouse button."
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                Button("Mouse 4") {
                    store.setShortcut(mouse: 3, label: "Mouse 4")
                    dismiss()
                }

                Button("Mouse 5") {
                    store.setShortcut(mouse: 4, label: "Mouse 5")
                    dismiss()
                }

                Button("Reset to ⌃⇧Space") {
                    store.setShortcut(label: "⌃⇧Space")
                    dismiss()
                }
            }

            Button("Cancel") {
                store.notifyWidget("Shortcut change cancelled.")
                dismiss()
            }
        ...
        .frame(width: 470)
        .onAppear {
            store.mouseHook?.stop()
            store.mouseHook = nil

            if let hotkey = store.hotkey {
                UnregisterEventHotKey(hotkey)
                store.hotkey = nil
            }

            if let local = store.localMouseMonitor {
                NSEvent.removeMonitor(local)
                store.localMouseMonitor = nil
            }
            ...
            monitor = NSEvent.addLocalMonitorForEvents(
                ...
                if event.type == .otherMouseDown {
                    store.setShortcut(
                        ...
                        label: "Mouse \(event.buttonNumber + 1)"
                    )
                    dismiss()
                    return nil
                }
                ...
                guard
                    ...
                    hint = "Include Control, Option, or Command with your key."
                    return nil
                }
